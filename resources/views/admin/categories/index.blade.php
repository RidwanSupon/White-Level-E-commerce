...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    <div class="bg-slate-950 p-6 rounded-3xl border border-slate-800 h-fit space-y-4">
        <h3 class="font-bold text-white text-base border-b border-slate-800 pb-3">Create Category</h3>
        <form action="{{ route('admin.categories.store') }}" method="POST" class="space-y-4">
            ...
            <div>
                <label class="block text-xs font-semibold text-slate-300 mb-1">Parent Category</label>
                <select name="parent_id" class="w-full bg-slate-900 border border-slate-800 rounded-xl px-4 py-2 text-sm text-white outline-none focus:border-brand-500">
            ...
            <div>
                ...
                <input type="url" name="image" placeholder="https://..." class="w-full bg-slate-900 border border-slate-800 rounded-xl px-4 py-2 text-sm text-white outline-none focus:border-brand-500">
            </div>
            <button type="submit" class="w-full py-2.5 bg-brand-500 hover:bg-brand-600 text-white font-bold text-xs rounded-xl shadow-md">Add Category</button>
        </form>
    </div>

    <div class="lg:col-span-2 bg-slate-950 rounded-3xl border border-slate-800 overflow-hidden">
        <div class="p-4 border-b border-slate-800 font-bold text-white text-base">Category Management</div>
            ...
            <tbody class="divide-y divide-slate-800">
                @foreach($categories as $cat)
                    <tr class="hover:bg-slate-900/40">
                        ...
                        <td class="py-3 px-4 text-slate-400">{{ $cat->parent?->name ?? '—' }}</td>
                        <td class="py-3 px-4 font-bold text-brand-400">{{ $cat->products_count }} items</td>
